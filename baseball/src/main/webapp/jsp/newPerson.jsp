...
<script>
	let countUsingId = 0; // 이미 사용중인 아이디
	let validDomain = false; // 도메인이 유효한지
	
	$('#input-domain').prop('readonly', true);
	$('#div-usingId, #div-invalidDomain').addClass('none');
	
	$('#img-kbo, #label-header').click(() => location.href = 'main');
	$('#btn-insert').click(checkValidation); // 회원가입시 값 입력됐는지 확인
	
	$('#input-phone').on('input', () => formatPhoneNumber($('#input-phone'))); // 핸드폰 번호 형식 세팅
	$('#input-id').on('input', () => checkId($('#input-id'))); // 사용중인 아이디인지 확인
	...
	function checkId($input){
		...
		$.ajax({
			...
			success:function(count){
				if(count != "0"){
					$('#div-usingId').removeClass('none').addClass('block');
				} else {
					$('#div-usingId').removeClass('block').addClass('none');
				}
				countUsingId = count;
			},
			error:function(err){
